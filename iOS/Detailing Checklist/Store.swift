...
final class Store: ObservableObject {
    ...
    init() {
        ...
        fileURL = support.appendingPathComponent("detailcheck_items.json")
        load()
        if items.isEmpty {
            seed()
        }
    ...
    func add(title: String, detail: String = "") {
        let item = ChecklistItem(title: title, detail: detail)
    ...
    func toggleDone(_ item: ChecklistItem) {
        guard let idx = items.firstIndex(where: { $0.id == item.id }) else { return }
    ...
    private func seed() {
        items = [
            ChecklistItem(title: "First entry", detail: "Sample seeded record"),
            ChecklistItem(title: "Second entry", detail: "Another sample record")
        ]
        save()
    }
